...
type XapiClient interface {
	...
	GetHighAvailabilityStatus() (*HighAvailability, error)
}
...
type HighAvailabilityInfo struct {
	State     string `xml:"state"`
	StateSync string `xml:"state-sync"`
	ManageIP  string `xml:"mgmt-ip"`
}
...
type HighAvailability struct {
	Enable string                `xml:"result>enabled"`
	Group  HighAvailabilityGroup `xml:"result>group"`
}
